...
def main_action(obs: Observation, our_idx: int) -> list[int]:
    ...
    if _energy_in_deck_estimate(ps) > 0:
    ...
    if (not state.energyAttached and (bench_cer_fire or bench_sol_fight_luna)
            ...
            and not _has_energy(act)):          # never attach second energy
        h = _hand(ps)
        for i, o in enumerate(opts):
            if o.type == OptionType.ATTACH and o.inPlayArea == AreaType.ACTIVE:
                if o.index is not None and o.index < len(h):
    ...
    opp_active_has_cape = (opp_act and any(t.id == Heros_Cape for t in opp_act.tools))
    if (act and act.id == Ceruledge_ex and _has_fire(act)
            and _count_hand(ps, Boss_Orders) > 0
            and not opp_active_has_cape):
        opp_bench = [p for p in opp_ps.bench if p is not None]
        act_prizes = _prize_value(opp_act.id) if opp_act else 0
        # Trigger A: active not KO-able, a bench target is
        trigger_a = (opp_act and opp_act.hp > dmg
                     and any(p.hp <= dmg for p in opp_bench))
        # Trigger B: a bench target worth more prizes than active AND is KO-able
        trigger_b = any(p.hp <= dmg and _prize_value(p.id) > act_prizes
                        for p in opp_bench)
...
def _prize_value(card_id: int) -> int:
    """Return prize card value: 2 for rule-box Pokémon (ex/V/etc.), 1 otherwise."""
    card = card_table.get(card_id)
    if card is None:
        return 1
    if getattr(card, 'ruleBox', False):
        return 2
...
def _energy_in_deck_estimate(ps) -> int:
    """Estimate energy cards remaining in deck (fire + fighting)."""
    all_ip = _all_in_play(ps)
    fire_out  = (_fire_in_hand(ps)
                 + sum(sum(1 for e in p.energyCards if e.id == Fire_Energy) for p in all_ip)
                 + _count_discard(ps, Fire_Energy))
    fight_out = (_fighting_in_hand(ps)
                 + sum(sum(1 for e in p.energyCards if e.id == Fighting_Energy) for p in all_ip)
                 + _count_discard(ps, Fighting_Energy))
    return max(0, 7 - fire_out) + max(0, 13 - fight_out)
...
def _gong_to_hand(ps, pairs) -> list[int]:
    ...
    if lun_ip and not sol_ip:
        r = pick(Solrock);
        if r: return r
    if sol_ip and lun_ip and fight_in_h >= 2:
        r = pick(Drilbur);
        if r: return r
    r = pick(Fighting_Energy)
    if r: return r
...
def _handle_discard(obs: Observation, our_idx: int) -> list[int]:
    ...
    if effect_id == Lunatone:
        # Lunar Cycle: must discard exactly 1 Fighting Energy from hand
        h = _hand(ps)
        fight = [i for i, o in enumerate(opts)
                 if o.type == OptionType.CARD and o.area == AreaType.HAND
                 and o.index is not None and o.index < len(h)
                 and h[o.index].id == Fighting_Energy]
        return fight[:1] if fight else list(range(min(1, len(opts))))
    ...
    if effect_id == Drilbur or (deck and effect_id is None):
        # Drilbur Dig Dig Dig: engine only presents Fighting Energy options;
        # take all available slots up to maxCount regardless of OptionType.
        return list(range(min(obs.select.maxCount, len(opts))))
    if effect_id == Ultra_Ball:
...
def _bb_discard(obs, our_idx, opts, deck) -> list[int]:
    """Brilliant Blender: choose up to 5 energy cards from deck to discard."""
    ...
    # Never discard non-energy cards.
    # If identification failed but options exist, take all (Blender only shows energy).
    if not sel and opts:
        return list(range(min(max_sel, len(opts))))
    return sel
