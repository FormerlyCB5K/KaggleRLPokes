# Agent authored by prod1gy on Kaggle.
# Source: https://www.kaggle.com/code/prod1gy/elo-1150-rule-based-agent-matchup-tests
# Imported here as a fixed evaluation opponent for the evolutionary pipeline.
...
class LucarioPolicy:
    ...
    def _score_option(self, option) -> float:
        ...
        if option.type == OptionType.RETREAT:
            return 2000 if _plan.attacker >= 1 else -1
        if option.type == OptionType.ATTACK:
            return 1100 if (option.attackId == MEGA_BRAVE) == (_plan.attack_index == 1) else 1000
        return 0
    ...
    def _score_active_choice(self, option, card: Pokemon | Card) -> float:
        ...
        if option.playerIndex != self.my_index:
            return 100 if option.index == _plan.target - 1 else 0
        score = len(card.energies) * 2
        if option.index == _plan.attacker - 1:
            score += 100
    ...
    def _score_to_hand(self, card: Pokemon | Card) -> float:
        ...
        if card.id == C.MAKUHITA:
            ...
            score += 30 if not _ability_used or not self.state.energyAttached else -1
        return score
    ...
    def _score_play_trainer(self, card: Card) -> float:
        if card.id == C.SWITCH:
            return 6000 if _plan.attacker > 0 else -1
        if card.id == C.PREMIUM_POWER_PRO:
            if self.state.supporterPlayed and _plan.remain_hp <= 0:
                return -1
        ...
        if card.id == C.BOSS_ORDERS:
            return 3200 if _plan.target >= 1 else -1
    ...
    def _score_gravity_mountain(self) -> float:
        opponent_has_stage2 = any(
            pokemon is not None and card_table[pokemon.id].stage2
    ...
    def _score_attach(self, option) -> float:
        ...
        if board_index == _plan.attacker and _plan.needs_energy:
            score += 200
        return score
    ...
    def _score_evolve(self, option) -> float:
        ...
        if not isinstance(pokemon, Pokemon):
            return 0
        if pokemon.id == C.MAKUHITA and _plan.target == 0:
            return -1
...
def agent(obs_dict: dict) -> list[int]:
    ...
    global _pre_turn, _ability_used, _plan
