"""
train-local.py — Local-machine variant of train.py for fast dev iteration.

Same PPO trainer as train.py (algorithm, opponent handling, checkpointing are
unchanged — see that file for the full design writeup). Differences here are
purely about running comfortably on a single local machine instead of the
cluster:

  * Small default EPISODES_PER_UPDATE / N_UPDATES / SAVE_EVERY so a run
    finishes in minutes, not hours.
  * wandb is OFF by default (pass --wandb to opt in); local dev usually
    doesn't want a cloud run per experiment.
  * Opponent selection supports both a single opponent (--opponent) and a
    weighted opponent pool (--opponent-pool name[:weight],..., e.g.
    "clefable:2,lucario:1,self:1,random:0.5") — identical flags to train.py.
  * Episode collection runs on CPU (the cg_download engine cannot run
    parallel workers, and CPU inference is fast enough for one episode at a
    time); the PPO gradient update batches on CUDA automatically whenever
    torch.cuda.is_available() — no flag needed, and the resolved device is
    printed at startup so it's obvious which one a given run used.
...
# ── Hyperparameters (local-scale defaults — override any of these on the CLI) ──
EPISODES_PER_UPDATE = 128       # rollout batch size (episodes) — small for a quick local loop
...
MAX_GRAD_NORM       = 0.5
LR                  = 1e-3
N_UPDATES           = 10000       # total update iterations
PRIZE_REWARD        = 0.0      # per-prize shaping reward; 0.0 = disabled, 0.01 = light shaping
DAMAGE_REWARD       = 0.01     # reward per 10 damage dealt; 0.0 = disabled
LOG_EVERY           = 1        # print/plot every N updates
SAVE_EVERY           = 10
OUT_DIR             = "out/REAL_train-new-architecture"
DESCRIPTION         = "running a training run on laptop while I sleep. lets see how it goes. No opponent pool. just rules."
...
# Episode collection is single-process CPU (the cg_download engine can't fork/
# spawn workers — see PROJECT_MEMORY); the PPO gradient update below always
# batches on this device, so a local GPU is used automatically when present.
DEVICE                 = torch.device("cuda" if torch.cuda.is_available() else "cpu")
WANDB_PROJECT          = "ceruledge-ppo"
WANDB_RUN_NAME         = ""     # "" = wandb auto-generates a name
USE_WANDB              = False  # local runs default OFF; pass --wandb to opt in

# ── CLI overrides ───────────────────────────────────────────────────────────────
import argparse as _ap
_p = _ap.ArgumentParser(description="Ceruledge PPO training (local dev)", add_help=True)
...
_p.add_argument("--wandb",                    action="store_true", default=False,
                help="enable wandb logging (off by default for local runs)")
...
USE_WANDB               = _WANDB_AVAILABLE and _args.wandb
...
def ppo_update(
    ...
    Batches are moved to `device` (CUDA when available — see DEVICE at module
    scope) before the forward/backward passes; the model itself is moved to
    `device` by the caller for the duration of the update.
...
def train():
    ...
    print(f"CUDA available: {torch.cuda.is_available()} — PPO gradient updates will run on: {DEVICE}",
          flush=True)
    ...
    for update in range(start_update, N_UPDATES + 1):
            ...
            # ── PPO update (on DEVICE — CUDA when available) ───────────────────
            model.train()
